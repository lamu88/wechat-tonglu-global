...
function submit_login_data(){
		...
		$.post(SITE_URL+'user.php',{action:'login',username:name,password:pas,vifcode:vcode,issave:issave,isauto:isauto},function(data){
			...
			if(data != ""){
				...
				JqueryDialog.Open('宝泰网登陆系统','<br /><p style="line-height:26px">登陆成功，系统将带你进入前一个页面！</p>',300,50);
		...
		$.ajax({
		   ...
		   success: function(data){ 
				...
				if(data.error==0){
					...
					JqueryDialog.Open('宝泰商城系统提醒你','<br /><p>'+data.message+'</p>',300,80);
				}
		   },
		...
		$.post(SITE_URL+'user.php',{action:'ressinfoop',id:id,type:type},function(data){
			if(type=='showupdate'){																	  
				if(data !="" && typeof(data) != 'undefined'){
					...
					JqueryDialog.Open('宝泰商城提醒你',data,400,300);
				}
			}else if(type=='delete'){
		  ...
		  $.ajax({
			   ...
			   success: function(data){
					...
					if(data.error==0){
						...
						JqueryDialog.Open('宝泰商城提醒你','<br /><img src="'+SITE_URL+'theme/images/error_icon.png" align="absmiddle"/>&nbsp;&nbsp;&nbsp;'+data.message,260,60);
					}
