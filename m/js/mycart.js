...
$('.delcartid').live('click',function(){
	if(confirm('您确实要把该商品移出购物车吗？')){
		...
		thisobj = $(this);
		createwindow();
		$.post(SITE_URL+'mycart.php',{action:'delcartid',id:ids},function(data){
				removewindow();
				if($(thisobj).attr("rel")=='mycarttop'){
					$(thisobj).parent().parent().parent().fadeOut();
				}else{
					$('.MYCART').hide();
					if(data !=""){
						$('.MYCART').html(data);
...
function change_number(obj){
	...
	createwindow();
	$.post(SITE_URL+'mycart.php',{action:'changeprice',id:id,number:numbers},function(data){ 
		removewindow();
		if(data.error==0){
			obj.parent().parent().find('.raturnprice').html('￥'+(price*numbers).toFixed(2));
			...
			alert(data.message);
			obj.parent().parent().find('input[name="goods_number"]').val(parseInt(numbers)-1);
