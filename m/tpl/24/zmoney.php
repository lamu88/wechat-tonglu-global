...
<table  width="100%" border="0" cellpadding="0" cellspacing="0" style="height:50px; border:1px solid #F0F0F0">
	<tr>
		...
		<p style="font-size:14px; padding-top:2px">财富排行榜</p>
		</td>
...
<?php if(!empty($rt['ulist']))foreach($rt['ulist'] as $k=>$row){?>
	<li style="padding:5px; border-bottom:1px solid #d8d8d8; position:relative">
		...
		<p style="line-height:23px"><?php echo $row['nickname'];?><br /><?php echo !empty($row['subscribe_time']) ? date('Y-m-d H:i:s',$row['subscribe_time']) : date('Y-m-d H:i:s',$row['reg_time']);?></p>
		</div>
		<div class="clear"></div>
		</a>
		<?php if($k<3){
