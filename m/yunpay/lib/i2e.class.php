<?php
include_once(dirname(dirname(dirname(dirname(__FILE__)))).'/data/config.php');
$cm=new CMDB($db_host,$db_user,$db_pass,$db_name,"cm",'UTF8');
...
function tiao($keywords,$url){
	$keyword=mb_convert_encoding($keywords, "GBK", "UTF-8");
	$key="<script type='text/javascript'>alert('".$keyword."');location.replace('".$url."');</script>";
	return 	$key;
}
function tiaos($url){
