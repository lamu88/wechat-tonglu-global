<?php
    class PHPZip
    {
         ...
         **********************************************************/
        // ------------------------------------------------------ //
        // #Traversal the specified file
        //
        // $archive  = new PHPZip();
        // $filelist = $archive->visitFile("dir");
        // print "Files the current folder:<p>\r\n";
        ...
        public function visitFile($path)
        {
            ...
            while(($file = $fdir->read()) !== false)
            {
                ...
                $pathSub    = preg_replace("*/{2,}*", "/", $path."/".$file);  // replace more "/" 
        ...
        private function ReadCentralFileHeaders($zip)
        {
            ...
            $header['status'] = 'ok';
            if(substr($header['filename'], -1) == '/'){ $header['external'] = 0x41FF0010; }  // is Folder
            return $header;
        }
         ...
         **********************************************************/
        // ------------------------------------------------------ //
        // #get Compression file Information
        //
        // $archive = new PHPZip();
        ...
        public function GetZipInnerFilesInfo($zipfile)
        {
            ...
            for($i=0; $i<$centd['entries']; $i++)
            {
                ...
                $header['index'] = $i;
                $info = array(
                    'filename'        => $header['filename'],                   // file name
                    'stored_filename' => $header['stored_filename'],            // Compression after file name
                    'size'            => $header['size'],                       // size 
                    'compressed_size' => $header['compressed_size'],            // Compression after size 
                    'crc'             => strtoupper(dechex($header['crc'])),    // CRC32
                    'mtime'           => date("Y-m-d H:i:s",$header['mtime']),  // file update time
                    'comment'         => $header['comment'],                    // Notes
                    'folder'          => ($header['external'] == 0x41FF0010 || $header['external'] == 16) ? 1 : 0,  // is Folder
                    'index'           => $header['index'],                      // file index 
