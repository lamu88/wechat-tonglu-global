...
<form id="form1" name="form1" method="post" action="">
     <table cellspacing="2" cellpadding="5" width="100%">
		...
		<tr>
			<td class="label" width="15%">总购物币</td>
			<td style="position:relative">
			<span class="thisgouwubi"><?php echo isset($rt['userinfo']['mygouwubi']) ? $rt['userinfo']['mygouwubi'] : '0';?></span>&nbsp;&nbsp;<a onclick="$('.user_gouwubi').toggle();" href="javascript:void(0)">[查看明细]</a>
			<div class="user_gouwubi" style=" z-index:9999;display:none;width:550px; border:1px solid #B4C9C6; position:absolute; left:-100px; top:30px; background-color:#ededed; padding:5px">
			<?php $this->element('ajax_user_gouwubi',array('rt'=>$rt));?>
			</div>
			&nbsp;&nbsp;<br /><font color="red">管理员修改用户购物币：</font><input type="text" size="10" onchange="change_user_points_money('<?php echo $rt['userinfo']['user_id'];?>',this,'gouwubi')"/> <em>输入正数增加，负数减少</em>
			</td>
		</tr>
		<!--
		<tr>
			<td class="label" width="15%">总购物币：</td>
			<td style="position:relative">
			<span class="thispoints"><?php echo isset($rt['userinfo']['mygouwubi']) ? $rt['userinfo']['mygouwubi'] : '0';?></span>&nbsp;&nbsp;<!--<a onclick="$('.user_point').toggle();" href="javascript:void(0)">[查看明细]</a>-->
...
function change_user_points_money(uid,thisobj,type){
				...
				if(typeof(data)!='undefined' && data!=""){
					...
					if(parseInt(data)>0){
						if(type=='money'){
							...
							$(thisobj).parent().find('.thisgouwubi').html(data);
						}
					}
					alert("操作成功！");
				}else{
 ...
 function get_usergouwubi_page_list(page,uid){
  		createwindow();
		$.post('user.php',{action:'mygouwubi',page:page,uid:uid},function(data){
			removewindow();
			if(data !="" && typeof(data)!='undefined'){
				$('.user_gouwubi').html(data);
			}
		});
}
</script>
