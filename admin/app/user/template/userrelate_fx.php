...
<div class="contentbox">
	...
	?>
	<tr>
	<td>&nbsp;&nbsp;&nbsp;<?php if($row['zcount'] > 0){?><a href="<?php echo ADMIN_URL.'user.php?type=userrelate_own_fx&id='.$row['uid'];?>">【<?php echo $rankname ?>】+&nbsp;<?php echo !empty($row['nickname']) ? $row['nickname'] : '未关注';echo "<font color=bule>(".(empty($row['zcount']) ? '0' : $row['zcount']).")</font>";?></a><?php }else{?><a href="javascript:;">&nbsp;<?php echo !empty($row['nickname']) ? $row['nickname'] : '未关注';echo "<font color=red>(".(empty($row['zcount']) ? '0' : $row['zcount']).")</font>";?></a><?php } ?>
	</td>
