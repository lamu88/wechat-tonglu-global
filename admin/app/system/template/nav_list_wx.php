<div class="contentbox">
     <table cellspacing="2" cellpadding="5" width="100%">
		...
		<th colspan="9" align="left">自定义导航栏列表</th>
     	...
     	<tr>
        	<th>名称</th><th>图标</th><th>是否显示</th><th>位置</th><th>排序</th><th>操作</th>
        </tr>
		<?php 
		if(!empty($rts)){
		foreach($rts as $row){
        ...
        <tr>
			...
			<td><img src="<?php echo SITE_URL.$row['img'];?>" width="50" height="50" /></td>
			...
			<td>
	<a href="systemconfig.php?type=nav_info_wx&id=<?php echo $row['id'];?>" title="编辑"><img src="<?php echo $this->img('icon_edit.gif');?>" title="编辑"/></a>&nbsp;
 ...
 $('.quxuanall').click(function (){
  ...
  $('.gids').click(function(){ 
   ...
   $('.delnav').click(function(){
		...
		if(confirm("确定删除吗？")){
			createwindow();
			$.post('<?php echo $thisurl;?>',{action:'ajax_delnav_wx',ids:id},function(data){
				removewindow();
				if(data == ""){
					thisobj.hide(300);
				}else{
   	...
   	$('.activeop').live('click',function(){
		...
		type = $(this).attr('lang');
		obj = $(this);
		$.post('<?php echo $thisurl;?>',{action:'ajax_active_wx',active:star,cid:tid,type:type},function(data){
			if(data == ""){
				if(star == 1){
					id = 0;
	...
	function update(id, object){
	   ...
	   $.post('<?php echo $thisurl;?>',{action:'ajax_vieworder_wx',id:id,val:editval},function(data){
			 obj.html(editval);
           	 $(object).unbind('click');
