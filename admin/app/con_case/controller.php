<?php
class Con_caseController extends Controller{
	//构造函数，自动新建对象
	...
	*/
	function catelist(){
		...
		$catelist = $this->action('common','get_cate_tree',0,'case');
		$this->set('catelist',$catelist);
		$this->template('con_case_cate_list');
	}
	...
	function cateinfo($type='cateadd',$id=0){ 
		...
		if($type=='cateedit'){
			...
			if(!empty($_POST)){
				...
				$this->action('system','add_admin_log','添加模板案例分类:'.$_POST['cat_name']);
				$this->action('common','showdiv',$this->getthisurl());
				$_POST['cat_img'] = "";
				$rt = $_POST;
			}
		}
		//$catelist = $this->get_cate_tree(0,'case');
		$this->set('catelist',$this->action('common','get_cate_tree',0,'case'));
		
		$this->set('rt',$rt);
		$this->set('type',$type);
		$this->template('con_case_cate_info');
	}
	
	/*
	* 内容列表
	*/
	function newlist($type='case'){
		$w="";
		if(!empty($type)){
		...
		$list = 10;
		$start = ($page-1)*$list;
		$sql = "SELECT COUNT(article_id) FROM `{$this->App->prefix()}article` {$w}";
	...
	*/
	function newinfo($type='newadd',$id=0){
		...
		if($type=='newedit'){
			if($id==0){
				$this->jump('con_case.php?type=newlist',0,'非法操作，ID为空！'); exit;
			}else{
				if(!empty($_POST)){
					...
					$this->action('system','add_admin_log','修改案例文章:'.$_POST['article_title']);
					$this->action('common','showdiv',$this->getthisurl());
				}
			...
			if(!empty($_POST)){
				...
				$this->action('system','add_admin_log','添加案例文章:'.$_POST['article_title']);
				$this->action('common','showdiv',$this->getthisurl());
				$rt = $_POST;
			}
		} 
		...
		$this->set('rt',$rt);
		$this->set('catids',$this->action('common','get_cate_tree',0,'case'));
		$this->set('type',$type);
		$this->template('con_case_info');
	}
	...
	//ajax检查模板颜色分类
	function ajax_check_color_cat_name($data=array()){
		$cat_id = $data['cat_id'];
		$cat_name = $data['cat_name'];
		if(empty($cat_name)){
			die("分类名称不能为空！");
		}
		$sql = "SELECT cat_id,cat_name FROM `{$this->App->prefix()}case_color_cate` WHERE cat_name='{$cat_name}' LIMIT 1";
		$rt = $this->App->find($sql);
		$cname = isset($rt[0]['cat_name']) ? $rt[0]['cat_name'] : "";
		$cid = isset($rt[0]['cat_id']) ? $rt[0]['cat_id'] : "";
		if(!empty($cname) && $cid!=$cat_id){
	...
	//ajax删除模板的颜色分类
	function ajax_delcate_color($ids=0){
		if(empty($ids)) die("非法删除，删除ID为空！");
		$id_arr = @explode('+',$ids);
		if(!empty($id_arr)){
			foreach($id_arr as $id){
				$data=array('colorid'=>0);
				if($id<=0) continue;
				$this->App->update('article',$data,'colorid',$id);  //更新模板文章下的colorid
				$this->App->delete('case_color_cate','cat_id',$id); //删除颜色分类
			}
		} 
	...
	function ajax_alt_activeop($data=array()){
		...
		if($data['type']=='is_show'){
			$sdata['is_show']= $data['active'];
			$this->action('system','add_admin_log','修改案例文章审核状态:ID为=>'.$data['cid']);
		}else if($data['type']=='is_top'){
			$sdata['is_top']= $data['active'];
			$this->action('system','add_admin_log','修改案例文章置顶状态:ID为=>'.$data['cid']);
		}
	...
	//ajax颜色分类的排序
	function ajax_vieworder_color($data=array()){
		if(empty($data['id'])) return "50";
		$sdata['vieworder'] = empty($data['val']) ? 50 : $data['val'];
		$this->App->update('case_color_cate',$sdata,'cat_id',$data['id']);
	}
