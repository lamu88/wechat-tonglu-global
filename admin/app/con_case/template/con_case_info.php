...
<form id="form1" name="form1" method="post" action="">
	 ...
	 <tr>
		<th colspan="2" align="left"><?php echo $type=='newedit' ? '修改' : '添加';?>品牌</th>
	</tr>
	<tr>
		<td class="label" width="15%">日期:</td>
		<td width="85%">
	 ...
	 <!-- <tr>
		<td class="label" width="15%">模板颜色分类:</td>
		<td width="85%">
		...
		?>
        <option value="<?php echo $row['cat_id'];?>"  <?php echo $row['cat_id']==$rt['colorid'] ? 'selected="selected"' : '';?>><?php echo $row['cat_name'];?></option>
		 <?php
		 }//end foreach
		} ?>
		</select> 
...
//jQuery(document).ready(function($){
	$('.new_save').click(function(){
		...
		if(art_title=='undefined' || art_title==""){
			$('.article_title_mes').html("案例标题不能为空！");
