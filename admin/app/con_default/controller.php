...
class Con_defaultController extends Controller{
	//构造函数，自动新建对象
	...
	*/
	function catelist(){
		...
		$catelist = $this->action('common','get_cate_tree',0,'about');
		$this->set('catelist',$catelist);
		$this->template('con_default_cate_list');
	}
	...
	function cateinfo($type='cateadd',$id=0){ 
		...
		if($type=='cateedit'){
			...
			if(!empty($_POST)){
				...
				$this->action('system','add_admin_log','添加系统默认分类:'.$_POST['cat_name']);
				$this->action('common','showdiv',$this->getthisurl());
				$_POST['cat_img'] = "";
				$rt = $_POST;
			}
		}
		//$catelist = $this->get_cate_tree(0,'about');
		$this->set('catelist',$this->action('common','get_cate_tree',0,'about'));
		
		$this->set('rt',$rt);
		$this->set('type',$type);
		$this->template('con_default_cate_info');
	}
	
	/*
	* 内容列表
	*/
	function newlist($type='about'){
		$w="";
		if(!empty($type)){
		...
		$this->template('con_default_list');
	}
	...
	*/
	function newinfo($type='newadd',$id=0){
		...
		if($type=='newedit'){
			if($id==0){
				$this->jump('con_default.php?type=newlist',0,'非法操作，ID为空！'); exit;
			}else{
				if(!empty($_POST)){
					...
					$this->action('system','add_admin_log','修改系统默认文章:'.$_POST['article_title']);
					$this->action('common','showdiv',$this->getthisurl());
				}
			...
			if(!empty($_POST)){
				...
				$this->action('system','add_admin_log','添加系统默认文章:'.$_POST['article_title']);
				$this->action('common','showdiv',$this->getthisurl());
				$rt = $_POST;
			}
		}
		$this->set('rt',$rt);
		$this->set('catids',$this->action('common','get_cate_tree',0,'about'));
		$this->set('type',$type);
		$this->template('con_default_info');
	}
	...
	function ajax_delarticle($ids=0){
		...
		$sql = "SELECT article_img FROM `{$this->App->prefix()}article_cate` WHERE article_id IN(".@implode(',',$id_arr).")";
		$imgs = $this->App->findcol($sql);
		if(!empty($imgs)){
			foreach($imgs as $img){
				if(empty($img)) continue;
		...
		$this->action('system','add_admin_log','删除模板案例文章：'.@implode(',',$id_arr));
	}
	//分类列表激活
	function ajax_active($data=array()){
		...
		if($data['type']=='show_in_nav'){
			//添加到导航栏表
			$sql = "SELECT cid FROM `{$this->App->prefix()}nav` WHERE cid ='{$data[cid]}' LIMIT 1";
	...
	function ajax_alt_activeop($data=array()){
		...
		if($data['type']=='is_show'){
			$sdata['is_show']= $data['active'];
			$this->action('system','add_admin_log','修改系统默认文章审核状态:ID为=>'.$data['cid']);
		}else if($data['type']=='is_top'){
			$sdata['is_top']= $data['active'];
			$this->action('system','add_admin_log','修改系统默认文章置顶状态:ID为=>'.$data['cid']);
		}else if($data['type']=='show_in_nav'){
			//添加到导航栏表
			$sql = "SELECT cid FROM `{$this->App->prefix()}nav` WHERE cid ='{$data[cid]}' AND ctype ='a' LIMIT 1";
			$checkvar = $this->App->findvar($sql);
			$sdata['show_in_nav']= $data['active'];
			...
			if(empty($checkvar)){
				$cdata['ctype'] = 'a';
				$cdata['cid'] = $data['cid'];
				$cdata['name'] = $this->App->findvar("SELECT article_title FROM `{$this->App->prefix()}article` WHERE article_id ='{$data[cid]}' LIMIT 1");
				$cdata['is_show'] = 1;
				$cdata['is_opennew'] = 0;
				$cdata['url'] = 'article.php?id='.$data['cid'];
				$this->App->insert('nav',$cdata);
				unset($cdata);
			 }else{ 
			 	$sql = "DELETE FROM `{$this->App->prefix()}nav` WHERE cid='$checkvar' AND ctype='a'";
			 	$this->App->query($sql);
			 }
			 
			$this->action('system','add_admin_log','修改系统默认文章显示在导航菜单:ID为=>'.$data['cid']);
		}
	...
	function ajax_vieworder($data=array()){
		...
		$this->App->update('article',$sdata,'article_id',$data['id']);
	}
	//分类排序
	function ajax_cate_vieworder($data=array()){
		if(empty($data['id'])) return "50";
		$sdata['vieworder'] = empty($data['val']) ? 50 : $data['val'];
