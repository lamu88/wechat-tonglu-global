<div class="contentbox">
	 ...
	 <tr>
		<th colspan="9" align="left">系统默认分类</th>
	</tr>
    <tr>
...
<td><span class="vieworder" id="<?php echo $row['id'];?>"><?php echo $row['vieworder'];?></span></td>
  <td><?php echo !empty($row['addtime']) ? date('Y-m-d',$row['addtime']) : "无知";?></td>
	<td>
	<a href="con_default.php?type=cateedit&id=<?php echo $row['id'];?>" title="编辑"><img src="<?php echo $this->img('icon_edit.gif');?>" title="编辑"/></a>&nbsp;
			...
			if(!empty($row['cat_id'])){ 
				...
				<td><span class="vieworder" id="<?php echo $rows['id'];?>"><?php echo $rows['vieworder'];?></span></td>
				  <td><?php echo !empty($rows['addtime']) ? date('Y-m-d',$rows['addtime']) : "无知";?></td>
					<td>
					<a href="con_default.php?type=cateedit&id=<?php echo $rows['id'];?>" title="编辑"><img src="<?php echo $this->img('icon_edit.gif');?>" title="编辑"/></a>&nbsp;
							...
							?>
								...
								<td><span class="vieworder" id="<?php echo $rowss['id'];?>"><?php echo $rowss['vieworder'];?></span></td>
								  <td><?php echo !empty($rowss['addtime']) ? date('Y-m-d',$rowss['addtime']) : "无知";?></td>
									<td>
									<a href="con_default.php?type=cateedit&id=<?php echo $rowss['id'];?>" title="编辑"><img src="<?php echo $this->img('icon_edit.gif');?>" title="编辑"/></a>&nbsp;
  ...
  //批量删除
   ...
   $('.delcateid').click(function(){
	...
	function update(id, object){
	   ...
	   $.post('<?php echo $thisurl;?>',{action:'cate_vieworder',id:id,val:editval},function(data){ 
			 obj.html(editval);
           	 $(object).unbind('click');
