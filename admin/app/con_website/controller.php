...
class Con_websiteController extends Controller{
	//构造函数，自动新建对象
	...
	*/
	function catelist(){
		...
		$catelist = $this->action('common','get_cate_tree',0,'web');
		$this->set('catelist',$catelist);
		$this->template('con_website_cate_list');
	}
	...
	function cateinfo($type='cateadd',$id=0){ 
		...
		if($type=='cateedit'){
			...
			if(!empty($_POST)){
				...
				$this->action('system','add_admin_log','添加网站建设下的分类:'.$_POST['cat_name']);
				$this->action('common','showdiv',$this->getthisurl());
				$_POST['cat_img'] = "";
				$rt = $_POST;
			}
		...
		$this->set('catelist',$this->action('common','get_cate_tree',0,'web'));
		
		$this->set('rt',$rt);
		$this->set('type',$type);
		$this->template('con_website_cate_info');
	}
	
	/*
	* 内容列表
	*/
	function newlist($type='web'){
		$w="";
		if(!empty($type)){
		...
		$this->template('con_website_list');
	}
	...
	*/
	function newinfo($type='newadd',$id=0){
		...
		if($type=='newedit'){
			if($id==0){
				$this->jump('con_website.php?type=newlist',0,'非法操作，ID为空！'); exit;
			}else{
				if(!empty($_POST)){
					...
					$this->action('system','add_admin_log','修改网站建设下的文章:'.$_POST['article_title']);
					$this->action('common','showdiv',$this->getthisurl());
				}
			...
			if(!empty($_POST)){
				...
				$this->action('system','add_admin_log','添加网站建设下的文章:'.$_POST['article_title']);
				$this->action('common','showdiv',$this->getthisurl());
				$rt = $_POST;
			}
		}
		$this->set('rt',$rt);
		$this->set('catids',$this->action('common','get_cate_tree',0,'web'));
		$this->set('type',$type);
		$this->template('con_website_info');
	}
	...
	function ajax_alt_activeop($data=array()){
		...
		if($data['type']=='is_show'){
			$sdata['is_show']= $data['active'];
			$this->action('system','add_admin_log','修改网站建设文章审核状态:ID为=>'.$data['cid']);
		}else if($data['type']=='is_top'){
			$sdata['is_top']= $data['active'];
			$this->action('system','add_admin_log','修改网站建设文章置顶状态:ID为=>'.$data['cid']);
		}
