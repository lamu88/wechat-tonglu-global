...
class Con_clientlistController extends Controller{
	//构造函数，自动新建对象
	...
	*/
	function catelist(){
		...
		$catelist = $this->action('common','get_cate_tree',0,'customer');
		$this->set('catelist',$catelist);
		$this->template('con_clientlist_cate_list');
	}
	...
	*/
	function cateinfo($type='cateadd',$id=0){ 
		...
		if($type=='cateedit'){
			if($id==0){
				$this->jump('con_clientlist.php?type=catelist',0,'非法操作，ID为空！'); exit;
			}else{
				if(!empty($_POST)){
					$_POST['meta_keys'] = !empty($_POST['meta_keys']) ? str_replace(array('，','。','.'),',',$_POST['meta_keys']) : "";
					$_POST['uptime'] = time();
					$this->App->update('article_cate',$_POST,'cat_id',$id);
					$this->action('system','add_admin_log','修改客户列表分类:'.$_POST['cat_name']);
			...
			if(!empty($_POST)){
				$_POST['addtime'] = time();
				$_POST['uptime'] = time(); //print_r($_POST); exit;
				$_POST['meta_keys'] = !empty($_POST['meta_keys']) ? str_replace(array('，','。','.'),',',$_POST['meta_keys']) : "";
				$this->App->insert('article_cate',$_POST);
				$this->action('system','add_admin_log','添加客户列表分类:'.$_POST['cat_name']);
				$this->action('common','showdiv',$this->getthisurl());
				$_POST['cat_img'] = "";
				$rt = $_POST;
			}
		}
		//$catelist = $this->get_cate_tree(0,'customer');
		$this->set('catelist',$this->action('common','get_cate_tree',0,'customer'));
		
		$this->set('rt',$rt);
		$this->set('type',$type);
		$this->template('con_clientlist_cate_info');
	}
	
	/*
	* 内容列表
	*/
	function newlist($type='customer'){
		$w="";
		if(!empty($type)){
		...
		$this->template('con_clientlist_list');
	}
	...
	*/
	function newinfo($type='newadd',$id=0){
		...
		if($type=='newedit'){
			if($id==0){
				$this->jump('con_clientlist.php?type=newlist',0,'非法操作，ID为空！'); exit;
			}else{
				if(!empty($_POST)){
					...
					$this->action('system','add_admin_log','修改客户列表文章:'.$_POST['article_title']);
					$this->action('common','showdiv',$this->getthisurl());
				}
			...
			if(!empty($_POST)){
				...
				$this->action('system','add_admin_log','添加客户列表文章:'.$_POST['article_title']);
				$this->action('common','showdiv',$this->getthisurl());
				$rt = $_POST;
			}
		}
		$this->set('rt',$rt);
		$this->set('catids',$this->action('common','get_cate_tree',0,'customer'));
		$this->set('type',$type);
		$this->template('con_clientlist_info');
	}
	...
	function ajax_delarticle($ids=0){
		...
		$this->action('system','add_admin_log','删除客户列表文章：'.@implode(',',$id_arr));
	}
	//分类列表激活
	...
	function ajax_alt_activeop($data=array()){
		...
		if($data['type']=='is_show'){
			$sdata['is_show']= $data['active'];
			$this->action('system','add_admin_log','修改客户列表文章审核状态:ID为=>'.$data['cid']);
		}else if($data['type']=='is_top'){
			$sdata['is_top']= $data['active'];
			$this->action('system','add_admin_log','修改客户列表文章置顶状态:ID为=>'.$data['cid']);
		}
