...
clearfile(SYS_PATH_PHOTOS."temp".DS.(empty($_SESSION['adminname'])?'admin':$_SESSION['adminname'])); //遍历文件，将该库里的文件删除
//循环调用多层文件夹文件
...
['size'][$key]/1024)."Kb,大于规定的".($file_size_max/1024)."Kb,上传失败!')</script>"; 
	...
	echo "<script>window.parent.Finish('".$_msg."','".$_REQUEST[cateid]."+".$_REQUEST[bandid]."');</script>";	
?>
